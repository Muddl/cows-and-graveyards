...
# Usage:
#   godot --headless -s tools/lint_shaders.gd                          # Lint all shaders
#   godot --headless -s tools/lint_shaders.gd -- res://path.gdshader   # Lint single shader
#
...
func _init() -> void:
	...
	if args.size() == 0:
		...
		print("Usage: godot --headless -s tools/lint_shaders.gd [-- path/to/shader.gdshader]")
		quit(1)
		return
