...
namespace CowsGraveyards.DevTools;
...
public partial class DevTools : Node
{
    ...
    public CommandResult HandleCommand(string action, JsonElement args)
    {
        ...
        if (_handlers.TryGetValue(action, out var handler))
        {
            ...
            {
                return new CommandResult(false, ex.Message, new { exception = ex.ToString() });
            }
        }

        return new CommandResult(false, $"Unknown command: {action}");
    }
    ...
    private void CheckForCommands()
    {
        ...
        try
        {
            ...
            var result = HandleCommand(command.Action, command.Args);
    ...
    private CommandResult CmdInputActions(JsonElement args)
    {
        ...
        foreach (var action in actions)
        {
            ...
            var includeBuiltin = args.ValueKind != JsonValueKind.Undefined
                && args.TryGetProperty("include_builtin", out var builtinEl) && builtinEl.GetBoolean();
